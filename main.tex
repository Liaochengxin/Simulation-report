...
本项目完成了从简单人体建模的Gamma/Proton治疗研究到微观细胞 BNCT 的 Geant4 模拟。

Q1：gamma 能量沉积弥散，无明显峰值；proton 可通过能量调节将 Bragg peak 置于肿瘤深度，45 MeV 与当前几何匹配最佳，质子治疗的关键优化变量是入射能量。

Q2：在 $^{10}\mathrm{B}$ 仅富集于肿瘤细胞的理想模型中，$\alpha$/$^{7}\mathrm{Li}$ 短射程产物形成明显的细胞尺度剂量局域化；$^{10}\mathrm{B}$ 浓度影响反应活性与靶细胞剂量，uniform 分布更有利于提高肿瘤核剂量；相对中子注量主要控制累计剂量和二次粒子产额，不改变由 $^{10}\mathrm{B}$ 分布决定的细胞选择性。
